...
from .city_plan import build_city_plan
...
def generate_roads(cfg: dict[str, Any], out_path: Path) -> dict[str, Any]:
    plan = build_city_plan(cfg)
    geo = generate_roads_impl(plan, cfg)
    write_json(out_path, geo)
    return geo

def generate_runways(cfg: dict[str, Any], out_path: Path) -> dict[str, Any]:
    plan = build_city_plan(cfg)
    geo = generate_runways_taxiways(plan, cfg["map"]["bbox"])
    write_json(out_path, geo)
    return geo

def generate_context_layers(cfg: dict[str, Any], output_dir: Path) -> None:
    plan = build_city_plan(cfg)
    water = generate_water(plan, cfg["map"]["bbox"])
    open_space = generate_open_spaces(plan, cfg["map"]["bbox"])
    campuses = generate_campuses(plan, cfg["map"]["bbox"])
